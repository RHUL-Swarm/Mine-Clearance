...
to go_exp
  ...
  set current_mine_start_count 0
  set current_searcher_start_count 0

  let rounds 0
  let freeze_event_count 0
  let stuck_count 0
  let initial_number_of_searchers 10 ; resets searcher loop
  let initial_number_of_mines 10
  set number_of_searchers 10
  set number_of_mines_remaining 10;20;10

  let mine_starting_amount 10
  ...
  let to_many_sticks FALSE

  while [mine_starting_amount < 101]
  [
    set current_mine_start_count mine_starting_amount

    while [number_of_searchers < 101]
      [
        set new_file_name (word "RW_Baseline_Searchers_" number_of_searchers "__Mines_" mine_starting_amount "_" date ".csv")
        ...
        set current_searcher_start_count number_of_searchers

        while [((rounds < number_of_rounds) and (to_many_freezes = FALSE) and (to_many_sticks = FALSE))]
        [
          ...
          [
            ...
            [
              ...
              print (word"freeze_event_count:" freeze_event_count)
              set freeze_state FALSE
            ...
            [
              ...
              print (word"stuck_count:" stuck_count)
              set stuck_state FALSE
              if (stuck_count >= stuck_count_Max_Amount)
              [
                SET to_many_sticks TRUE
              ]
            ]
          ]
        ]
        ...
        SET to_many_sticks FALSE

        set ave_count_to_clear_all_mines (count_to_clear_all_mines / number_of_rounds)

        file-print " "
        file-print (word " Searchers: ," number_of_searchers ", Mines: ," mine_starting_amount ", Ave Time to Clear Mines: ," ave_count_to_clear_all_mines ", Freeze Event Count: ," freeze_event_count ", Stuck Count: ," stuck_count)

        print (word "ave_count_to_clear_all_mines: " ave_count_to_clear_all_mines)
...
to move_searchers
  ask searchers
  [
    if (current_state = 0) ; i.e. Foraging
    [
...
to scent_check
  ...
  [
    ...
    ifelse ((scent_ahead > 0) or (scent_right > 0) or (scent_left > 0)) ; current state searching
    [
      if (current_state = 0)
      [
